# Estudio del estado: Veracruz

## Instrucciones para Claude Code

Eres responsable de generar el estudio completo del estado de **Veracruz**. Esto significa producir 8 archivos en esta carpeta (`estados/03_veracruz/`), ejecutando uno tras otro los prompts genéricos correspondientes y aplicándolos al contexto de **Veracruz**.
...
### Contexto pre-cargado de Veracruz
...
- Petroquímica (Coatzacoalcos, Minatitlán)
- Puerto y logística (Veracruz, Tuxpan)
- Agroindustria (caña, cítricos, café)
- Acero y siderurgia (Lázaro Cárdenas histórico, pero también AHMSA)
- Pesca y alimentos del mar
- Construcción y vivienda

#### Pistas de familias empresariales relevantes
- Familia Loyo (transporte, construcción)
- Familia Rouanet (puerto, logística)
- Familia Castellanos (agroindustria)
- Familia Jiménez (Veracruz)
...
- Pemex Petroquímica
- Innophos (cliente ya en Tier A del estudio Excel)
- Tenaris TAMSA (Veracruz puerto)
- Grupo Cementos Chihuahua
- API Veracruz (Administración Portuaria)
- Hutchison Ports
...
- API Veracruz
- AMACARGA
- Cámara de Industria de Coatzacoalcos
...
1. **Ranking Expansión 500** (`data/expansion/LCG_500_Empresas_Expansion_2025.csv`) — empresas cuya sede o operación principal sea Veracruz
2. **CRM HubSpot** (`data/crm/hubspot_empresas_AAA_AA.csv`) — filtra por `Estado de la República` == "Veracruz"
...
1. **La narrativa económica del estado en un párrafo** — el "elevator pitch" de qué es Veracruz hoy
...
5. **5 insights clave** que Fray debería tener en mente sobre Veracruz
...
> Ejecuta estados/03_veracruz/prompt.md completo, generando los 8 archivos y actualizando el README del estado.
```

O paso a paso, archivo por archivo:

```bash
> Ejecuta el paso 1 de estados/03_veracruz/prompt.md (panorama económico).
