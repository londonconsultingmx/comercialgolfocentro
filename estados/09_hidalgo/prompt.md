# Estudio del estado: Hidalgo

## Instrucciones para Claude Code

Eres responsable de generar el estudio completo del estado de **Hidalgo**. Esto significa producir 8 archivos en esta carpeta (`estados/09_hidalgo/`), ejecutando uno tras otro los prompts genéricos correspondientes y aplicándolos al contexto de **Hidalgo**.
...
### Contexto pre-cargado de Hidalgo
...
- Cemento (Cruz Azul, Tula)
- Refinación (refinería Tula de Pemex)
- Manufactura (parques industriales en Tepeji, Atotonilco)
- Logística (cercanía a CDMX, eje Pachuca-CDMX)
- Agroindustria (cebada, alfalfa, ganado)
- Construcción

#### Pistas de familias empresariales relevantes
- Familia Mestre Gavito
- Cooperativa Cruz Azul (gobierno colectivo complejo)
...
- Bimbo (planta Lerma cercana)
- Volkswagen (planta de motores en Silao... no, eso es Guanajuato)
...
1. **Ranking Expansión 500** (`data/expansion/LCG_500_Empresas_Expansion_2025.csv`) — empresas cuya sede o operación principal sea Hidalgo
2. **CRM HubSpot** (`data/crm/hubspot_empresas_AAA_AA.csv`) — filtra por `Estado de la República` == "Hidalgo"
...
1. **La narrativa económica del estado en un párrafo** — el "elevator pitch" de qué es Hidalgo hoy
...
5. **5 insights clave** que Fray debería tener en mente sobre Hidalgo
...
> Ejecuta estados/09_hidalgo/prompt.md completo, generando los 8 archivos y actualizando el README del estado.
```

O paso a paso, archivo por archivo:

```bash
> Ejecuta el paso 1 de estados/09_hidalgo/prompt.md (panorama económico).
