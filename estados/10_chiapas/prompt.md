# Estudio del estado: Chiapas

## Instrucciones para Claude Code

Eres responsable de generar el estudio completo del estado de **Chiapas**. Esto significa producir 8 archivos en esta carpeta (`estados/10_chiapas/`), ejecutando uno tras otro los prompts genéricos correspondientes y aplicándolos al contexto de **Chiapas**.
...
### Contexto pre-cargado de Chiapas
...
- Agroindustria (café — el estado #1 productor, cacao, frutas tropicales)
- Energía (presas hidroeléctricas, gas)
- Turismo (San Cristóbal, Palenque)
- Pesca
- Ganadería
- Petróleo (limitado pero presente)

#### Pistas de familias empresariales relevantes
- Familia Albores
- Familia Sabines (política-empresarial histórica)
- Familia Domínguez
- Familia Coello
...
- CFE (presas)
- Bachoco (operaciones)
- Productores de café organizados

#### Cámaras y asociaciones esperadas
- COPARMEX Chiapas
- CCE Chiapas
- Cámara de Comercio Tuxtla Gutiérrez
- AMECAFE Chiapas
...
1. **Ranking Expansión 500** (`data/expansion/LCG_500_Empresas_Expansion_2025.csv`) — empresas cuya sede o operación principal sea Chiapas
2. **CRM HubSpot** (`data/crm/hubspot_empresas_AAA_AA.csv`) — filtra por `Estado de la República` == "Chiapas"
...
1. **La narrativa económica del estado en un párrafo** — el "elevator pitch" de qué es Chiapas hoy
...
5. **5 insights clave** que Fray debería tener en mente sobre Chiapas
...
> Ejecuta estados/10_chiapas/prompt.md completo, generando los 8 archivos y actualizando el README del estado.
```

O paso a paso, archivo por archivo:

```bash
> Ejecuta el paso 1 de estados/10_chiapas/prompt.md (panorama económico).
