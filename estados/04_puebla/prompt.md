# Estudio del estado: Puebla

## Instrucciones para Claude Code

Eres responsable de generar el estudio completo del estado de **Puebla**. Esto significa producir 8 archivos en esta carpeta (`estados/04_puebla/`), ejecutando uno tras otro los prompts genéricos correspondientes y aplicándolos al contexto de **Puebla**.
...
### Contexto pre-cargado de Puebla
...
- Automotriz (clúster Volkswagen, Audi, sus tier-1s y tier-2s)
- Manufactura (textiles, papel, vidrio)
- Bienes de consumo (CBC, Softys, RG Refresco)
- Agroindustria (huevo, lácteos)
- Educación superior y servicios (UDLAP, Tec Puebla, BUAP)

#### Pistas de familias empresariales relevantes
- Familia Romero Deschamps (políticamente y sindicalmente vinculada)
- Familia Henaine (Henaine = ferreterías, retail)
- Familia Yarrington (no — error, esa es Tamaulipas)
- Familia Carrancá (banca histórica)
- Familia Hank (de Hank Rhon — político-empresarial)
...
- Audi México
- CBC (Continental Beverage Company)
- Softys (Sigdo Koppers)
- RG Refresco Embotelladora de México
- Novares Group
- Unicar Plastics
...
- COPARMEX Puebla
- Cámara Nacional de la Industria de Transformación delegación Puebla
- Cluster Automotriz de Puebla y Tlaxcala (CLAUT)
- Asociación Mexicana de la Industria Automotriz (AMIA) — presencia
...
1. **Ranking Expansión 500** (`data/expansion/LCG_500_Empresas_Expansion_2025.csv`) — empresas cuya sede o operación principal sea Puebla
2. **CRM HubSpot** (`data/crm/hubspot_empresas_AAA_AA.csv`) — filtra por `Estado de la República` == "Puebla"
...
1. **La narrativa económica del estado en un párrafo** — el "elevator pitch" de qué es Puebla hoy
...
5. **5 insights clave** que Fray debería tener en mente sobre Puebla
...
> Ejecuta estados/04_puebla/prompt.md completo, generando los 8 archivos y actualizando el README del estado.
```

O paso a paso, archivo por archivo:

```bash
> Ejecuta el paso 1 de estados/04_puebla/prompt.md (panorama económico).
